...
void AtomVecSphereKokkos::copy(int i, int j, int delflag)
{
  ...
  //TODO: why is this not in AtomVecSphere::copy
  //h_torque(j,0) = h_torque(i,0);
  //h_torque(j,1) = h_torque(i,1);
  //h_torque(j,2) = h_torque(i,2);
...
struct AtomVecSphereKokkos_PackBorder {
  ...
  const typename ArrayTypes<DeviceType>::t_float_1d _radius;
  const typename ArrayTypes<DeviceType>::t_float_1d _rmass;
  X_FLOAT _dx,_dy,_dz;
  ...
  if (pbc_flag != 0) {
    ...
    if(space==Host) {
      AtomVecSphereKokkos_PackBorder<LMPHostType,1> f(
        ...
        iswap,h_x,h_tag,h_type,h_mask,h_radius,h_rmass,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecSphereKokkos_PackBorder<LMPDeviceType,1> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        iswap,d_x,d_tag,d_type,d_mask,d_radius,d_rmass,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
...
void AtomVecSphereKokkos::data_atom(double *coord, tagint imagetmp,
                                    char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
